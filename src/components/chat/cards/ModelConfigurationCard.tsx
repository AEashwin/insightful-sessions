        ...
        <section className="overflow-hidden rounded-lg border border-primary/20 bg-background/90 shadow-sm">
          ...
          <div className="max-h-[360px] overflow-auto">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-primary/10 text-[11px] uppercase text-primary">
...
function RangeInputs({ values, onChange }: { values: string[]; onChange: (index: number, value: string) => void }) { return <div className="grid min-w-[210px] grid-cols-3 gap-1.5">{["Min", "Max", "Inc"].map((label, index) => <Input key={label} aria-label={label} value={values[index]} onChange={(event) => onChange(index, event.target.value)} className="h-8 text-xs" />)}</div>; }
